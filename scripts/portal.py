...
class Portal:
    ...
    def draw(self, surf, offset=(0, 0)):
        ...
        pulse = 0.1 * math.sin(t * 0.05)

        # Outer frame (solid)
        frame_rect = pygame.Rect(x - 2, y - 2, w + 4, h + 4)
        pygame.draw.rect(surf, self.outline_color, frame_rect, border_radius=4)

        # Inner glow (static but slightly shifting brightness)
        main = (
            min(255, int(self.main_color[0] * (1 + pulse))),
            min(255, int(self.main_color[1] * (1 + pulse))),
            min(255, int(self.main_color[2] * (1 + pulse)))
        )
        pygame.draw.rect(surf, main, (x, y, w, h), border_radius=3)
